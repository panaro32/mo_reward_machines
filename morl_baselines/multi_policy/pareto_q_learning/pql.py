...
class PQL(MOAgent):
    ...
    def train(
        ...
        while self.global_step < total_timesteps:
            ...
            while not (terminated or truncated) and self.global_step < total_timesteps:
                ...
                for crm_state, crm_action, crm_reward, crm_next_state, crm_done in info['crm']:
                    crm_state = int(np.ravel_multi_index(crm_state, self.env_shape))
                    crm_next_state = int(np.ravel_multi_index(crm_next_state, self.env_shape))
                    self.counts[crm_state, crm_action] += 1
                    self.non_dominated[crm_state][crm_action] = self.calc_non_dominated(crm_next_state)
                    self.avg_reward[crm_state, crm_action] += (crm_reward - self.avg_reward[crm_state, crm_action]) / self.counts[crm_state, crm_action]
        ...
        return self.get_local_pcs(state=self.initial_state)

    def _eval_all_policies(self, env: gym.Env) -> List[np.ndarray]:
        """Evaluate all learned policies by tracking them."""
        pf = []
        for vec in self.get_local_pcs(state=self.initial_state):
    ...
    def track_policy(self, vec, env: gym.Env, tol=1e-3):
        ...
        while not (terminated or truncated):
            ...
            state, reward, terminated, truncated, info = env.step(closest_action)

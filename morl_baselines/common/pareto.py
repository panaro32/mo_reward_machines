...
def get_non_dominated(candidates: set, size=50) -> set:
    ...
    extra = len(non_dominated) - size
    if extra <= 0:
        return non_dominated
    non_dominated = np.array(list(non_dominated))
    pruned_non_dominated = non_dominated[crowding(non_dominated).argsort()[extra:]]
    return {tuple(point) for point in pruned_non_dominated}
